...
//      to exactly 1.
//  12. BSPMap implements IBSPCollisionWorld. AABBPhysics takes
//      IBSPCollisionWorld* instead of BSPMap*. Friend declaration
//      removed — physics only accesses collision geometry.
// ============================================================
#pragma once
...
namespace nova
{
    ...
    // Raw brush structures for collision (Quake 2)
    struct BSPRawBrush
    {
        int32_t firstBrushSide; // index into BrushSides lump
        int32_t numBrushSides;  // number of brush sides
        int32_t contents;       // contents flags (SOLID, WATER, etc.)
    };
    static_assert(sizeof(BSPRawBrush) == 12, "BSPRawBrush must be 12 bytes");

    struct BSPRawBrushSide
    {
        uint16_t plane; // index into Planes lump
        int16_t texinfo;
    };
    static_assert(sizeof(BSPRawBrushSide) == 4, "BSPRawBrushSide must be 4 bytes");
    ...
    // ---- BSP Map ----
    class BSPMap : public IBSPCollisionWorld
    {
    public:
        ~BSPMap();
        ...
        const BSPLeaf *leaves() const override { return m_leaves.data(); }
        const BSPPlane *planes() const override { return m_planes.data(); }
        const BSPBrush *brushes() const override { return m_brushes.data(); }
        const BSPBrushSide *brushSides() const override { return m_brushSides.data(); }
        const int *leafBrushes() const override { return m_leafBrushes.data(); }

        int nodeCount() const override { return (int)m_nodes.size(); }
        int leafCount() const override { return (int)m_leaves.size(); }
        int planeCount() const override { return (int)m_planes.size(); }
        int brushCount() const override { return (int)m_brushes.size(); }
        int brushSideCount() const override { return (int)m_brushSides.size(); }
    ...
    private:
        ...
        std::vector<BSPBrushSide> m_brushSides;
        ...
        struct Surface
        {
            uint32_t firstVertex = 0; // explicit first vertex (not derived from index array)
